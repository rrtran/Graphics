...
GLuint compile_shaders(void)
{
	...
	static const GLchar* vertex_shader_source[] =
	{
		...
		"	int y = 0;\n"
		"	float y_offset;\n"
		"	float x_offset = bitfieldExtract(gl_InstanceID, 0, 10) * 0.02f;\n"
		"	float z_offset = bitfieldExtract(gl_InstanceID, 10, 10) * 0.02f;\n"
		"	if ( bitfieldExtract(gl_InstanceID, 10, 10) > 0 )\n"
		"	{\n"
		"		int i;\n"
		"		for (i = 0; i < bitfieldExtract(gl_InstanceID, 10, 10); i++) {\n"  // i < 0 --> i < 1
		"			y++;\n"
		"		}\n"
		"		x_offset = (bitfieldExtract(gl_InstanceID, 0, 10) % 100) * 0.02f;\n"
		"	}\n"
		"	y_offset = y * 0.05f;\n"
		"	gl_Position = position + vec4(x_offset, y_offset, z_offset, 0.0f);\n"
		"}\n"
	};
...
public:
	...
	void render(double currentTime) 
	{
		...
		static const GLfloat vertex_positions[] =
		{
			...
			-0.99f, -1.0f, 0.5f,  // 1
			-0.99f, -0.98f, 0.5f, // 2
			-1.0f, -0.98f, 0.5f,  // 3     4 5
			-1.0f, -0.96, 0.5f,	  // 4     3 2
			-0.99f, -0.96f, 0.5f   // 5     0 1
		};
		...
		glDrawElementsInstanced(GL_TRIANGLES, 12, GL_UNSIGNED_SHORT, 0, 50000);
